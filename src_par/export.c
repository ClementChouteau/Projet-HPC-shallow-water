...
MPI_File *create_file(void) {
	...
	MPI_File_open(MPI_COMM_WORLD, "test.out", MPI_MODE_CREATE|MPI_MODE_WRONLY, MPI_INFO_NULL, &f);
...
void export_step(MPI_File *f, int t) {
	...
	// EN MODE BLOC ON DOIT FAIRE PLUSIEURS WRITES CAR IL Y A LES BORDS DANS LE TABLEAU

	for (int j = 0; j < ; j++)
		MPI_File_write_at(f, offset+j*size_y, &HFIL(t, id_x*(size_x/q), id_y*(size_y/q)+j), size_y, MPI_DOUBLE, NULL);
}
